...
Both search engines support the filetype operator:
  ...
  # Bug fix: args is a Namespace, not subscriptable; use args.url
  ...
  def pprint_url(urls, title='URLs'):
    ...
    for key in sorted(stats.keys()):
      ...
      for pdir in sorted(dic.keys()):
        pdirurls = dic[pdir]
        print ('%s%s' % (key, pdir))
        print ('\n'.join(sorted(set(pdirurls))))
        print ('\n')

# Bug fix: return a list so the result can be iterated multiple times
def remove_tags(l):
  return list(map(lambda a: re.sub(r'<[^>]+>', '', a), l))
...
class BingSE(BaseSE):
  ...
  def request(self, query):
    ...
    while not stop:
      ...
      # Bug fix: use https instead of http
      code, data = self.fetch('https://www.bing.com' + uri, self.headers)
...
def main():
  # Bug fix: parse_options returns a tuple (args, domain); unpack it
  args, domain = parse_options(sys.argv)
